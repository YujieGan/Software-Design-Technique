...
public class WatchList implements Bingeable<Watchable> {
	 ...
	 *
	 * @param w2
	 			the watchlist to compare
	 * @return if two watchlists are the same
	 */
	public boolean equals(WatchList w2) {
		if( w2 == null )
		{ return false; }
		else if( w2 == this )
		{ return true; }
		else if (aList.size() != w2.aList.size())
		{ return false; }
		else {
			for (int i=0; i<aList.size(); i++){
				if (! aList.get(i).equals(w2.aList.get(i))){
					return false;
				}
			}
			return true;
