...
public class TVShow implements Watchable, Bingeable<Episode> {
	...
	private static final List<TVShow> TVShows = new ArrayList<>();
	
	/**
	 * Creates a TVShow instance with required metadata about the show.
	 *
	 * @param pTitle
	...
	public static TVShow get(String pTitle, Language pLanguage, String pStudio) {
		...
		for (TVShow t : TVShows) {
			if (t.aTitle.equals(pTitle)) {
				return t;
			}
		}
		TVShow tvs = new TVShow(pTitle,pLanguage,pStudio);
		TVShows.add(tvs);
		return tvs;
	}
