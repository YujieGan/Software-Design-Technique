...
public class Episode implements Sequenceable<Episode>, Cloneable {
	...
	/**
	 * Override clone()
	 * @return a clone of the episode
	 */
	public Episode clone() {
		try
		{
			Episode eClone = (Episode) super.clone();
			eClone.aCast = (Map<String, String>) super.clone();
			eClone.aTags = (Map<String, String>) super.clone();
			return eClone;
	...
	/**
	 * Clone the episode and set a new path
	 * @param pPath the new path of cloned episode
	 * @pre pPath != null
	 * @return a cloned episode with the given path
	 */
	public Episode clone(File pPath) {
		assert pPath != null;
		Episode result = this.clone();
		result.aPath = pPath;
		return result;
	}
